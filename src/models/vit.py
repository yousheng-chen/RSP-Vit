...
def clone_module_list(module: nn.Module, n: int, share_weights: bool):
    if share_weights:
        return nn.ModuleList([module for _ in range(n)])
    return nn.ModuleList([copy.deepcopy(module) for _ in range(n)])


def build_transformer_layer_list(transformer_layer, n: int, share_weights: bool):
    if isinstance(transformer_layer, nn.ModuleList):
        if len(transformer_layer) != n:
            raise ValueError(f"Expected {n} transformer layers, got {len(transformer_layer)}")
        return transformer_layer
    if isinstance(transformer_layer, Sequence) and not isinstance(transformer_layer, (str, bytes)):
        if len(transformer_layer) != n:
            raise ValueError(f"Expected {n} transformer layers, got {len(transformer_layer)}")
        return nn.ModuleList(list(transformer_layer))
    return clone_module_list(transformer_layer, n, share_weights=share_weights)
...
class ResNetStemPatchEmbeddings(nn.Module):
    """
    A lightweight CNN-style stem before patch projection.

    This keeps the ViT token interface unchanged, but replaces the single
    large patchifying convolution with a few small convolutions that first
    extract local patterns.
    """

    def __init__(self, d_model: int, patch_size: int, in_channels: int, stem_channels=(32, 64)):
        super().__init__()
        if patch_size <= 0:
        ...
        reduction = 2 ** len(stem_channels)
        if patch_size % reduction != 0:
            raise ValueError(
                f"patch_size={patch_size} is not divisible by the stem downsampling factor {reduction}. "
                "Choose a patch_size compatible with the number of stem stages."
            )
        ...
        proj_patch = patch_size // reduction
        self.proj = nn.Conv2d(current_channels, d_model, kernel_size=proj_patch, stride=proj_patch)
...
class VisionTransformer(nn.Module):
    ...
    def __init__(self, 
                 ...
                 share_transformer_weights: bool = False,
                 use_depth_embeddings: bool = False):
        """
        ...
        * `classification` is the [classification head](#ClassificationHead).
        ...
        self.use_depth_embeddings = use_depth_embeddings
        self.transformer_layers = build_transformer_layer_list(
            transformer_layer,
            n_layers,
            share_weights=share_transformer_weights,
        )
        layer_size = self.transformer_layers[0].size
        if self.use_depth_embeddings:
            self.depth_embeddings = nn.Parameter(torch.zeros(n_layers, layer_size), requires_grad=True)
        else:
            self.register_parameter("depth_embeddings", None)
        ...
        self.cls_token_emb = nn.Parameter(torch.randn(1, 1, layer_size), requires_grad=True)
    ...
    def forward(self, x: torch.Tensor):
        ...
        * `x` is the input image of shape `[batch_size, channels, height, width]`
        ...
        for layer_idx, layer in enumerate(self.transformer_layers):
            if self.depth_embeddings is not None:
                depth_emb = self.depth_embeddings[layer_idx].view(1, 1, -1)
                x = x + depth_emb
            x = layer(x=x, mask=None, layer_idx=layer_idx)
