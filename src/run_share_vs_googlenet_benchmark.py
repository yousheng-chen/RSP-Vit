...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Run shared ViT vs official GoogLeNet across all noisy datasets and generate summary charts."
    )
    ...
    parser.add_argument("--use-pretrained-googlenet", action="store_true")
...
def build_wide_rows(rows: list[dict]) -> list[dict]:
    ...
    for noise_db in sorted(grouped.keys()):
        ...
        googlenet_row = grouped[noise_db].get("googlenet")
        wide_rows.append(
            {
                "noise_db": noise_db,
                "share_test_acc": share_row.get("test_acc") if share_row else "",
                "googlenet_test_acc": googlenet_row.get("test_acc") if googlenet_row else "",
                "share_best_val_acc": share_row.get("best_val_acc") if share_row else "",
                "googlenet_best_val_acc": googlenet_row.get("best_val_acc") if googlenet_row else "",
                "share_total_params": share_row.get("total_params") if share_row else "",
                "googlenet_total_params": googlenet_row.get("total_params") if googlenet_row else "",
                "share_training_time_sec": share_row.get("training_time_sec") if share_row else "",
                "googlenet_training_time_sec": googlenet_row.get("training_time_sec") if googlenet_row else "",
                "share_run_dir": share_row.get("run_dir_name") if share_row else "",
                "googlenet_run_dir": googlenet_row.get("run_dir_name") if googlenet_row else "",
            }
        )
    return wide_rows
...
def write_markdown_table_benchmark(rows: list[dict], out_path: Path) -> None:
    lines = [
        "# Shared ViT vs GoogLeNet Noise Benchmark Summary",
        "",
...
def plot_paper_overview(rows: list[dict], out_path: Path) -> None:
    ...
    fig.suptitle("Shared ViT vs GoogLeNet: Robustness, Efficiency, and Capacity", fontsize=14, fontweight="bold", y=0.98)
...
def generate_summary_outputs(rows: list[dict], benchmark_dir: Path) -> None:
    ...
    wide_fields = [
        ...
        "googlenet_test_acc",
        "share_best_val_acc",
        "googlenet_best_val_acc",
        "share_total_params",
        "googlenet_total_params",
        "share_training_time_sec",
        "googlenet_training_time_sec",
        "share_run_dir",
        "googlenet_run_dir",
    ]
...
def run_googlenet_experiment(
    ...
    train_official_cnn(
        ...
        use_pretrained=args.use_pretrained_googlenet,
...
def main() -> None:
    ...
    benchmark_name = args.benchmark_name or f"share_vs_googlenet_benchmark_{time.strftime('%Y%m%d_%H%M%S')}"
