...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Reuse the best historical shared-ViT runs and train official MnasNet1.0 across all noisy datasets."
    )
    ...
    parser.add_argument("--use-pretrained-mnasnet", action="store_true")
...
def build_wide_rows(rows: list[dict]) -> list[dict]:
    ...
    for noise_db in sorted(grouped.keys()):
        ...
        mnasnet_row = grouped[noise_db].get("mnasnet1_0")
        wide_rows.append(
            {
                "noise_db": noise_db,
                "share_test_acc": share_row.get("test_acc") if share_row else "",
                "mnasnet1_0_test_acc": mnasnet_row.get("test_acc") if mnasnet_row else "",
                "share_best_val_acc": share_row.get("best_val_acc") if share_row else "",
                "mnasnet1_0_best_val_acc": mnasnet_row.get("best_val_acc") if mnasnet_row else "",
                "share_total_params": share_row.get("total_params") if share_row else "",
                "mnasnet1_0_total_params": mnasnet_row.get("total_params") if mnasnet_row else "",
                "share_training_time_sec": share_row.get("training_time_sec") if share_row else "",
                "mnasnet1_0_training_time_sec": mnasnet_row.get("training_time_sec") if mnasnet_row else "",
                "share_run_dir": share_row.get("run_dir_name") if share_row else "",
                "mnasnet1_0_run_dir": mnasnet_row.get("run_dir_name") if mnasnet_row else "",
            }
        )
    return wide_rows
...
def write_markdown_table_benchmark(rows: list[dict], out_path: Path) -> None:
    lines = [
        "# Shared ViT vs MnasNet1.0 Noise Benchmark Summary",
        "",
...
def plot_paper_overview(rows: list[dict], out_path: Path) -> None:
    ...
    for mode in MODEL_ORDER:
        ...
        ax_main.plot(xs, ys, linewidth=2.5, markersize=7, marker=MODEL_MARKERS[mode], color=MODEL_COLORS[mode], label=MODEL_LABELS[mode])
    ...
    fig.suptitle("Shared ViT vs MnasNet1.0: Robustness, Efficiency, and Capacity", fontsize=14, fontweight="bold", y=0.98)
...
def generate_summary_outputs(rows: list[dict], benchmark_dir: Path) -> None:
    ...
    wide_fields = [
        ...
        "mnasnet1_0_test_acc",
        "share_best_val_acc",
        "mnasnet1_0_best_val_acc",
        "share_total_params",
        "mnasnet1_0_total_params",
        "share_training_time_sec",
        "mnasnet1_0_training_time_sec",
        "share_run_dir",
        "mnasnet1_0_run_dir",
    ]
...
def _derive_noise_label(metrics: dict, metrics_path: Path) -> str | None:
    if metrics.get("noise_label"):
        return str(metrics["noise_label"])
    ...
    if data_dir:
        try:
            ...
            return None
    try:
        return Path(metrics_path.parent.name).name
    except Exception:
        return None


def _resolve_share_search_dirs(share_source_root: Path, share_source_dirs: list[str] | None) -> list[Path]:
    if not share_source_dirs:
        return [share_source_root]

    resolved_dirs: list[Path] = []
    for item in share_source_dirs:
        path = Path(item)
        if not path.is_absolute():
            path = (share_source_root / path).resolve()
        resolved_dirs.append(path)
    return resolved_dirs


def select_best_share_run(
    noise_label: str,
    share_source_root: Path,
    share_source_dirs: list[str] | None,
    selection_metric: str,
) -> tuple[Path, dict]:
    candidates: list[tuple[float, Path, dict]] = []
    for search_dir in _resolve_share_search_dirs(share_source_root, share_source_dirs):
        if not search_dir.exists():
            continue
        for metrics_path in search_dir.rglob("metrics.json"):
            try:
                metrics = load_metrics(metrics_path)
            except Exception:
                continue

            is_share = metrics.get("mode") == "share" or metrics.get("share_transformer_weights") is True
            if not is_share:
                continue

            candidate_noise_label = _derive_noise_label(metrics, metrics_path)
            if candidate_noise_label != noise_label:
                continue
            ...
            candidates.append((score_value, metrics_path, metrics))

    if not candidates:
        raise FileNotFoundError(
            f"No historical shared-ViT metrics found for noise='{noise_label}' under {share_source_root}"
        )

    candidates.sort(key=lambda item: item[0], reverse=True)
    _, best_metrics_path, best_metrics = candidates[0]
    return best_metrics_path.parent, best_metrics


def import_best_share_result(
    benchmark_name: str,
    benchmark_dir: Path,
    share_source_root: Path,
    share_source_dirs: list[str] | None,
    ...
    source_run_dir, metrics = select_best_share_run(
        noise_label=noise_label,
        share_source_root=share_source_root,
        share_source_dirs=share_source_dirs,
        selection_metric=selection_metric,
    )
    final_score = float(metrics.get("test_acc", metrics.get("best_val_acc", 0.0)))
    final_dir_name = build_final_dir_name("share", noise_label, run_index, final_score)
    final_run_dir = ensure_unique_path(benchmark_dir / final_dir_name)

    print(f"Importing best historical share run for {noise_label}: {source_run_dir}")
    shutil.copytree(str(source_run_dir), str(final_run_dir))

    final_metrics_path = final_run_dir / "metrics.json"
    copied_metrics = load_metrics(final_metrics_path)
    copied_metrics.update(
        {
            ...
            "source_run_name": metrics.get("run_name", source_run_dir.name),
        }
    )
    save_metrics(final_metrics_path, copied_metrics)

    return {
        "run_index": run_index,
        "mode": "share",
        "noise_label": noise_label,
        "noise_db": float(copied_metrics["noise_db"]),
        "test_acc": float(copied_metrics.get("test_acc", 0.0)),
        "best_val_acc": float(copied_metrics.get("best_val_acc", 0.0)),
        "test_loss": float(copied_metrics.get("test_loss", 0.0)),
        "total_params": int(copied_metrics.get("total_params", 0)),
        "trainable_params": int(copied_metrics.get("trainable_params", 0)),
        "training_time_sec": float(copied_metrics.get("training_time_sec", 0.0)),
        "data_dir": copied_metrics.get("data_dir", ""),
        "run_dir_name": final_run_dir.name,
        "run_dir": str(final_run_dir.resolve()),
    }


def run_mnasnet_experiment(
    benchmark_name: str,
    benchmark_dir: Path,
    ...
    train_official_cnn(
        ...
        use_pretrained=args.use_pretrained_mnasnet,
...
def main() -> None:
    ...
    benchmark_name = args.benchmark_name or f"share_vs_mnasnet_benchmark_{time.strftime('%Y%m%d_%H%M%S')}"
