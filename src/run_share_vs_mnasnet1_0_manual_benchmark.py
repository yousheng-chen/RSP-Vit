...
MODEL_ORDER = ("share", "mnasnet1_0_manual")
MODEL_LABELS = {
    "share": "Shared ViT",
    "mnasnet1_0_manual": "Manual MnasNet1.0",
}
MODEL_COLORS = {
    "share": "#1f4e79",
    "mnasnet1_0_manual": "#c75b12",
}
MODEL_MARKERS = {
    "share": "o",
    "mnasnet1_0_manual": "s",
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Reuse the best historical shared-ViT runs and train manual MnasNet1.0 across all noisy datasets."
    )
...
def build_wide_rows(rows: list[dict]) -> list[dict]:
    ...
    for noise_db in sorted(grouped.keys()):
        ...
        manual_row = grouped[noise_db].get("mnasnet1_0_manual")
        wide_rows.append(
            {
                "noise_db": noise_db,
                "share_test_acc": share_row.get("test_acc") if share_row else "",
                "mnasnet1_0_manual_test_acc": manual_row.get("test_acc") if manual_row else "",
                "share_best_val_acc": share_row.get("best_val_acc") if share_row else "",
                "mnasnet1_0_manual_best_val_acc": manual_row.get("best_val_acc") if manual_row else "",
                "share_total_params": share_row.get("total_params") if share_row else "",
                "mnasnet1_0_manual_total_params": manual_row.get("total_params") if manual_row else "",
                "share_training_time_sec": share_row.get("training_time_sec") if share_row else "",
                "mnasnet1_0_manual_training_time_sec": manual_row.get("training_time_sec") if manual_row else "",
                "share_run_dir": share_row.get("run_dir_name") if share_row else "",
                "mnasnet1_0_manual_run_dir": manual_row.get("run_dir_name") if manual_row else "",
            }
        )
    return wide_rows
...
def write_markdown_table_benchmark(rows: list[dict], out_path: Path) -> None:
    lines = [
        "# Shared ViT vs Manual MnasNet1.0 Noise Benchmark Summary",
        "",
...
def plot_paper_overview(rows: list[dict], out_path: Path) -> None:
    ...
    fig.suptitle("Shared ViT vs Manual MnasNet1.0: Robustness, Efficiency, and Capacity", fontsize=14, fontweight="bold", y=0.98)
...
def generate_summary_outputs(rows: list[dict], benchmark_dir: Path) -> None:
    ...
    wide_fields = [
        "noise_db",
        "share_test_acc",
        "mnasnet1_0_manual_test_acc",
        "share_best_val_acc",
        "mnasnet1_0_manual_best_val_acc",
        "share_total_params",
        "mnasnet1_0_manual_total_params",
        "share_training_time_sec",
        "mnasnet1_0_manual_training_time_sec",
        "share_run_dir",
        "mnasnet1_0_manual_run_dir",
    ]
...
def run_mnasnet_manual_experiment(
    benchmark_name: str,
    benchmark_dir: Path,
...
def main() -> None:
    ...
    benchmark_name = args.benchmark_name or f"share_vs_mnasnet1_0_manual_benchmark_{time.strftime('%Y%m%d_%H%M%S')}"
        ...
        for item in run_plan:
            ...
            try:
                ...
                if mode == "share":
                    ...
                    row = run_mnasnet_manual_experiment(
                        benchmark_name=benchmark_name,
                        benchmark_dir=benchmark_dir,
