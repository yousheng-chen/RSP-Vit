...
MODEL_ORDER = ("share", "mobilenet_v3_small")
MODEL_LABELS = {
    "share": "Shared ViT",
...
MODEL_COLORS = {
    "share": "#1f4e79",
    "mobilenet_v3_small": "#c75b12",
}
MODEL_MARKERS = {
    "share": "o",
    "mobilenet_v3_small": "s",
}


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Run shared-weight ViT vs official MobileNetV3-Small across all noisy datasets and generate summary charts."
    )
    ...
    parser.add_argument("--use-pretrained-mobilenetv3-small", action="store_true")
...
def build_wide_rows(rows: list[dict]) -> list[dict]:
    ...
    for noise_db in sorted(grouped.keys()):
        ...
        mobilenet_row = grouped[noise_db].get("mobilenet_v3_small")
        wide_rows.append(
            {
                "noise_db": noise_db,
                "share_test_acc": share_row.get("test_acc") if share_row else "",
                "mobilenet_v3_small_test_acc": mobilenet_row.get("test_acc") if mobilenet_row else "",
                "share_best_val_acc": share_row.get("best_val_acc") if share_row else "",
                "mobilenet_v3_small_best_val_acc": mobilenet_row.get("best_val_acc") if mobilenet_row else "",
                "share_total_params": share_row.get("total_params") if share_row else "",
                "mobilenet_v3_small_total_params": mobilenet_row.get("total_params") if mobilenet_row else "",
                "share_training_time_sec": share_row.get("training_time_sec") if share_row else "",
                "mobilenet_v3_small_training_time_sec": mobilenet_row.get("training_time_sec") if mobilenet_row else "",
                "share_run_dir": share_row.get("run_dir_name") if share_row else "",
                "mobilenet_v3_small_run_dir": mobilenet_row.get("run_dir_name") if mobilenet_row else "",
            }
        )
    return wide_rows
...
def write_markdown_table_benchmark(rows: list[dict], out_path: Path) -> None:
    lines = [
        "# Shared ViT vs MobileNetV3-Small Noise Benchmark Summary",
        "",
...
def plot_paper_overview(rows: list[dict], out_path: Path) -> None:
    ...
    fig.suptitle("Shared ViT vs MobileNetV3-Small: Robustness, Efficiency, and Capacity", fontsize=14, fontweight="bold", y=0.98)
...
def generate_summary_outputs(rows: list[dict], benchmark_dir: Path) -> None:
    ...
    wide_fields = [
        "noise_db",
        "share_test_acc",
        "mobilenet_v3_small_test_acc",
        "share_best_val_acc",
        "mobilenet_v3_small_best_val_acc",
        "share_total_params",
        "mobilenet_v3_small_total_params",
        "share_training_time_sec",
        "mobilenet_v3_small_training_time_sec",
        "share_run_dir",
...
def run_shared_vit_experiment(
    ...
    temp_run_dir = benchmark_dir / temp_run_name
...
def run_mobilenetv3_small_experiment(
    benchmark_name: str,
    benchmark_dir: Path,
    data_dir: Path,
    run_index: int,
    ...
    train_official_cnn(
        ...
        use_pretrained=args.use_pretrained_mobilenetv3_small,
...
def main() -> None:
    ...
    benchmark_name = args.benchmark_name or f"share_vs_mobilenetv3_small_benchmark_{time.strftime('%Y%m%d_%H%M%S')}"
        ...
        for item in run_plan:
            ...
            try:
                ...
                if mode == "share":
                    ...
                    row = run_mobilenetv3_small_experiment(
                        benchmark_name=benchmark_name,
                        benchmark_dir=benchmark_dir,
                        data_dir=data_dir,
                        run_index=run_index,
