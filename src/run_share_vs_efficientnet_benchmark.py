...
MODEL_ORDER = ("share", "efficientnet_b0")
MODEL_LABELS = {
    "share": "Shared ViT",
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Run shared-weight ViT vs official EfficientNet-B0 across all noisy datasets and generate summary charts."
    )
    ...
    parser.add_argument("--use-pretrained-efficientnet", action="store_true")
...
def build_wide_rows(rows: list[dict]) -> list[dict]:
    ...
    for noise_db in sorted(grouped.keys()):
        unshare_row = grouped[noise_db].get("share")
        efficientnet_row = grouped[noise_db].get("efficientnet_b0")
        wide_rows.append(
            {
                "noise_db": noise_db,
                "share_test_acc": unshare_row.get("test_acc") if unshare_row else "",
                "efficientnet_b0_test_acc": efficientnet_row.get("test_acc") if efficientnet_row else "",
                "share_best_val_acc": unshare_row.get("best_val_acc") if unshare_row else "",
                "efficientnet_b0_best_val_acc": efficientnet_row.get("best_val_acc") if efficientnet_row else "",
                "share_total_params": unshare_row.get("total_params") if unshare_row else "",
                "efficientnet_b0_total_params": efficientnet_row.get("total_params") if efficientnet_row else "",
                "share_training_time_sec": unshare_row.get("training_time_sec") if unshare_row else "",
                "efficientnet_b0_training_time_sec": efficientnet_row.get("training_time_sec") if efficientnet_row else "",
                "share_run_dir": unshare_row.get("run_dir_name") if unshare_row else "",
                "efficientnet_b0_run_dir": efficientnet_row.get("run_dir_name") if efficientnet_row else "",
            }
        )
    return wide_rows
...
def write_markdown_table_benchmark(rows: list[dict], out_path: Path) -> None:
    lines = [
        "# Shared ViT vs EfficientNet-B0 Noise Benchmark Summary",
        "",
...
def plot_paper_overview(rows: list[dict], out_path: Path) -> None:
    ...
    fig.suptitle("Shared ViT vs EfficientNet-B0: Robustness, Efficiency, and Capacity", fontsize=14, fontweight="bold", y=0.98)
...
def generate_summary_outputs(rows: list[dict], benchmark_dir: Path) -> None:
    ...
    wide_fields = [
        "noise_db",
        "share_test_acc",
        "efficientnet_b0_test_acc",
        "share_best_val_acc",
        "efficientnet_b0_best_val_acc",
        "share_total_params",
        "efficientnet_b0_total_params",
        "share_training_time_sec",
        "efficientnet_b0_training_time_sec",
        "share_run_dir",
        "efficientnet_b0_run_dir",
    ]
...
def run_efficientnet_experiment(
    ...
    train_official_cnn(
        ...
        use_pretrained=args.use_pretrained_efficientnet,
...
def main() -> None:
    ...
    benchmark_name = args.benchmark_name or f"share_vs_efficientnet_benchmark_{time.strftime('%Y%m%d_%H%M%S')}"
        ...
        for item in run_plan:
            ...
            try:
                ...
                if mode == "share":
                    ...
                    row = run_efficientnet_experiment(
                        benchmark_name=benchmark_name,
                        benchmark_dir=benchmark_dir,
                        checkpoint_root=checkpoint_root,
                        data_dir=data_dir,
