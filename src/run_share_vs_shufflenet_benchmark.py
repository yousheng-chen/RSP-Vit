...
MODEL_ORDER = ("share", "shufflenetv2")
MODEL_LABELS = {
    "share": "Shared ViT",
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Run shared-weight ViT vs official ShuffleNetV2 across all noisy datasets and generate summary charts."
    )
    ...
    parser.add_argument("--use-pretrained-shufflenet", action="store_true")
...
def build_wide_rows(rows: list[dict]) -> list[dict]:
    ...
    for noise_db in sorted(grouped.keys()):
        ...
        shufflenet_row = grouped[noise_db].get("shufflenetv2")
        wide_rows.append(
            {
                "noise_db": noise_db,
                "share_test_acc": share_row.get("test_acc") if share_row else "",
                "shufflenetv2_test_acc": shufflenet_row.get("test_acc") if shufflenet_row else "",
                "share_best_val_acc": share_row.get("best_val_acc") if share_row else "",
                "shufflenetv2_best_val_acc": shufflenet_row.get("best_val_acc") if shufflenet_row else "",
                "share_total_params": share_row.get("total_params") if share_row else "",
                "shufflenetv2_total_params": shufflenet_row.get("total_params") if shufflenet_row else "",
                "share_training_time_sec": share_row.get("training_time_sec") if share_row else "",
                "shufflenetv2_training_time_sec": shufflenet_row.get("training_time_sec") if shufflenet_row else "",
                "share_run_dir": share_row.get("run_dir_name") if share_row else "",
                "shufflenetv2_run_dir": shufflenet_row.get("run_dir_name") if shufflenet_row else "",
            }
        )
    return wide_rows
...
def write_markdown_table_benchmark(rows: list[dict], out_path: Path) -> None:
    lines = [
        "# Shared ViT vs ShuffleNetV2 Noise Benchmark Summary",
        "",
...
def plot_paper_overview(rows: list[dict], out_path: Path) -> None:
    ...
    fig.suptitle("Shared ViT vs ShuffleNetV2: Robustness, Efficiency, and Capacity", fontsize=14, fontweight="bold", y=0.98)
...
def generate_summary_outputs(rows: list[dict], benchmark_dir: Path) -> None:
    ...
    wide_fields = [
        "noise_db",
        "share_test_acc",
        "shufflenetv2_test_acc",
        "share_best_val_acc",
        "shufflenetv2_best_val_acc",
        "share_total_params",
        "shufflenetv2_total_params",
        "share_training_time_sec",
        "shufflenetv2_training_time_sec",
        "share_run_dir",
        "shufflenetv2_run_dir",
    ]
...
def run_shufflenet_experiment(
    ...
    train_official_cnn(
        ...
        use_pretrained=args.use_pretrained_shufflenet,
...
def main() -> None:
    ...
    benchmark_name = args.benchmark_name or f"share_vs_shufflenet_benchmark_{time.strftime('%Y%m%d_%H%M%S')}"
